...
import { FiFolder, FiSettings, FiTrash2, FiUser, FiRefreshCw, FiSearch, FiX, FiFileText } from 'react-icons/fi';
import styles from '@/app/manager/assets/workflows/ManagerWorkflowControll.module.scss';
import { getAllWorkflows, deleteWorkflow, approveWorkflowDeploy, rejectWorkflowDeploy } from '@/app/manager/api/workflow';
import {
...
import { showWorkflowDeleteConfirmKo } from '@/app/_common/utils/toastUtilsKo';
import ManagerWorkflowEditModal from '@/app/manager/components/workflows/ManagerWorkflowEditModal';
import ManagerWorkflowLogTab from '@/app/manager/components/workflows/ManagerWorkflowLogTab';
...
const ManagerWorkflowControll: React.FC = () => {
    ...
    const [editingWorkflow, setEditingWorkflow] = useState<ManagerWorkflow | null>(null);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [viewingLogWorkflow, setViewingLogWorkflow] = useState<ManagerWorkflow | null>(null);
    ...
    const fetchWorkflows = async () => {
        try {
            ...
            // 매니저용 API로 접근 가능한 워크플로우 가져오기
            const response = await getAllWorkflows(1, 1000, null);
            const workflowDetails = (response as any)?.workflows || [];

            const transformedWorkflows: ManagerWorkflow[] = workflowDetails.map((detail: any) => {
    ...
    const handleEdit = (workflow: ManagerWorkflow) => {
    ...
    const handleEditModalUpdate = (updatedWorkflow: ManagerWorkflow, updatedDeploy: {[key: string]: boolean | 'pending' | null}) => {
    ...
    const handleDeployApprove = async (workflow: ManagerWorkflow) => {
        try {
            await approveWorkflowDeploy(workflow.name, workflow);
    ...
    const handleDeployReject = async (workflow: ManagerWorkflow) => {
        try {
            await rejectWorkflowDeploy(workflow.name, workflow);
    ...
    const handleDelete = (workflow: ManagerWorkflow) => {
        showWorkflowDeleteConfirmKo(
            workflow.name,
            async () => {
                try {
                    await deleteWorkflow(workflow.user_id, workflow.name);
                    showDeleteSuccessToastKo({
    ...
    const handleViewLogs = (workflow: ManagerWorkflow) => {
        setViewingLogWorkflow(workflow);
    };

    const handleCloseLogTab = () => {
        setViewingLogWorkflow(null);
    };

    // 로그 탭이 열려있으면 로그 탭을 렌더링
    if (viewingLogWorkflow) {
        return (
            <div className={styles.container}>
                <div className={styles.logTabHeader}>
                    <button
                        className={styles.backButton}
                        onClick={handleCloseLogTab}
                    >
                        ← 목록으로 돌아가기
                    </button>
                    <div className={styles.workflowInfo}>
                        <h3>워크플로우 로그: {viewingLogWorkflow.name}</h3>
                        <p>워크플로우 ID: {viewingLogWorkflow.id}</p>
                    </div>
                </div>
                <ManagerWorkflowLogTab
                    workflowId={viewingLogWorkflow.id}
                    workflowName={viewingLogWorkflow.name}
                />
            </div>
        );
    }
                        ...
                        >
                            ...
                            <div className={styles.cardContent}>
                                ...
                                <div className={styles.workflowMeta}>
                                    ...
                                    <div className={styles.metaItem}>
                                        <span>노드: {workflow.nodeCount}</span>
                                    </div>
                                    {workflow.share_group && (
                                        <div className={styles.metaItem}>
                                            <span>그룹: {workflow.share_group}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
...
export default ManagerWorkflowControll;
