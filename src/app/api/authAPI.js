 ...
 */
const _signup = async (signupData) => {
    // 패스워드를 SHA256으로 해시화
 ...
 */
const _logout = async (token = null) => {
...
const _logoutWithCleanup = async (token = null) => {
    try {
        return await _logout(token);
    } catch (error) {
        // 로그아웃 실패해도 쿠키는 정리
        clearAllAuth();
        throw error;
...
export const logout = withErrorHandler(_logoutWithCleanup, 'Failed to logout');
 ...
 */
const _validateToken = async (token = null) => {
    const authToken = token || getAuthCookie('access_token');
...
const _validateTokenWithFallback = async (token = null) => {
    try {
...
export const validateToken = _validateTokenWithFallback;
 ...
 */
const _refreshToken = async (refreshToken = null) => {
 ...
 */
const _authenticatedFetch = async (url, options = {}) => {
    // 기본 헤더에 인증 헤더 추가
    const headers = {
        'Content-Type': 'application/json',
        ...getAuthHeaders(),
...
export const authenticatedFetch = withErrorHandler(_authenticatedFetch, 'Authenticated fetch failed');
 ...
 * @returns {Promise<Object>} 로그인 결과 (토큰 포함)
 */
const _createGuestAccountAndLogin = async () => {
...
const _createGuestAccountAndLoginWithErrorHandling = async () => {
    try {
        return await _createGuestAccountAndLogin();
    } catch (error) {
        throw new Error(
...
export const createGuestAccountAndLogin = withErrorHandler(_createGuestAccountAndLoginWithErrorHandling, 'Failed to create guest account or login');
