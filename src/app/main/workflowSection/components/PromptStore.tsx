...
import { getPromptsByLanguage, deletePrompt, downloadAllPrompts } from '@/app/_common/api/promptAPI';
...
const PromptStore: React.FC<PromptStoreProps> = ({ onPromptSelect, className }) => {
    ...
    const handleDownload = async () => {
        try {
            ...
            devLog.info('Downloading prompts with params:', {
                format,
                userId,
                language,
                publicAvailable,
            ...
            devLog.info('Prompts file blob received:', { size: blob.size, type: blob.type });
            ...
            link.download = `prompts_${timestamp}.${extension}`;
            ...
            devLog.info('Prompts file downloaded successfully');

            // 드롭다운 닫기
            setIsDownloadDropdownOpen(false);

        } catch (error) {
            devLog.error('Failed to download prompts file:', error);
                        ...
                        <div className={styles.downloadContainer} ref={downloadDropdownRef}>
                            ...
                            {isDownloadDropdownOpen && (
                                <div className={styles.downloadDropdown}>
                                    <h3 className={styles.dropdownTitle}>프롬프트 다운로드</h3>
