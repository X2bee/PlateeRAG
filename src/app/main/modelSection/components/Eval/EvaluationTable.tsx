...
import {
	...
	calculateJobScore,
	hasBaseModelResult
} from '@/app/main/modelSection/components/Eval/utils/eval';
import { showErrorToastKo, showValidationErrorToastKo } from '@/app/_common/utils/toastUtilsKo';
// import { deleteEvaluationJob, deleteMultipleEvaluationJobs } from '@/app/api/evalAPI'; // 가짜로 교체
import styles from '@/app/main/modelSection/assets/EvalTable.module.scss';
...
const EvaluationTable: React.FC<EvaluationTableProps> = ({
	...
	const handleDeleteJob = useCallback(async (jobId: string, event?: React.MouseEvent) => {
		...
		try {
			...
			showErrorToastKo('삭제 중 오류가 발생했습니다: ' + error.message);
		} finally {
	...
	const handleBulkDelete = useCallback(async () => {
		if (selectedJobs.size === 0) {
			showValidationErrorToastKo('삭제할 항목을 선택해주세요.');
			return;
		}
