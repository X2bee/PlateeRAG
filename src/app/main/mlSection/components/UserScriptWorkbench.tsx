...
const UserScriptWorkbench: React.FC<UserScriptWorkbenchProps> = ({
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (storedDrafts) {
                const parsed: ScriptDraft[] = JSON.parse(storedDrafts);
                setDrafts(parsed);
            }
        } catch (error) {
            console.warn('Failed to parse script drafts:', error);
        }
    ...
    }, [defaultPath, defaultTemplate, isInitialized]);
    ...
    const persistDrafts = useCallback(
        (nextDrafts: ScriptDraft[]) => {
            setDrafts(nextDrafts);
            try {
                localStorage.setItem(DRAFTS_KEY, JSON.stringify(nextDrafts));
            } catch (error) {
                console.warn('Failed to store drafts:', error);
            }
        },
        [setDrafts],
    );
    ...
    const handleSaveDraft = () => {
        ...
        const newDraft: ScriptDraft = {
            id: activeDraftId || (typeof crypto !== 'undefined' && crypto.randomUUID
                ? crypto.randomUUID()
                : `draft-${Date.now()}`),
            name,
            scriptPath,
        ...
        const nextDrafts = activeDraftId
            ? drafts.map((draft) => (draft.id === activeDraftId ? newDraft : draft))
            : [...drafts, newDraft];
    ...
    const handleDraftSelect = (event: ChangeEvent<HTMLSelectElement>) => {
        const value = event.target.value;
        if (!value) {
            return;
        }
        const draft = drafts.find((item) => item.id === value);
        if (!draft) return;
        setActiveDraftId(draft.id);
        setDraftName(draft.name);
        setScriptContent(draft.content);
        setScriptPath(draft.scriptPath);
        setRunConfig({
            ...draft.runConfig,
            feature_columns: [...(draft.runConfig.feature_columns || [])],
        });
    ...
    const handleDraftDelete = () => {
        ...
        const nextDrafts = drafts.filter((draftItem) => draftItem.id !== activeDraftId);
        persistDrafts(nextDrafts);
